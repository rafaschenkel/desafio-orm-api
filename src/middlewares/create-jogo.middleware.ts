...
export default async function createJogoMiddleware(
  ...
  if (!jogo.genero || typeof jogo.genero !== "string" || jogo.genero === "") {
    res.status(400).json({
      message: "O gênero do jogo é obrigatório ou está em um formato inválido",
    });
    return;
  }
  ...
  if (!jogo.tamanho || typeof jogo.tamanho !== "number" || jogo.tamanho <= 0) {
    res.status(400).json({
      message: "O tamanho do jogo é obrigatório ou está em um formato inválido",
    });
    return;
  }
